...
export default function SimoleonApp() {
  ...
  const [targetLinePct, setTargetLinePct] = useState(2);
  ...
  function tradeStock(symbol: string, mode: "buy" | "sell") {
    ...
    if (mode === "buy") {
      if (lot <= 0) return alert("Lot must be > 0");
  ...
  return <div className="container">
    ...
    <section className="card" style={{ marginTop: 16 }}>
      ...
      <div className="rowActions"><label>Auto Sell Timer</label><select value={autoSellSec} onChange={(e) => setAutoSellSec(Number(e.target.value))}><option value={5}>5s</option><option value={10}>10s</option><option value={30}>30s</option><option value={60}>1m</option></select><label>Custom Lot</label><input type="number" value={lotSize} onChange={(e)=>setLotSize(Number(e.target.value || 0))} style={{maxWidth:100}}/><label>Target ±%</label><input type="number" value={targetLinePct} onChange={(e)=>setTargetLinePct(Number(e.target.value || 0))} style={{maxWidth:90}}/><small>Stock tx currency follows your last ledger input: <b>{lastEntryCurrency}</b></small><small>Total saham: <b>§{portfolioValue.toFixed(2)}</b> | Total aset: <b>§{totalWealth.toFixed(2)}</b></small></div>
